...
当前数据会产生 4 个候选物业，DeepSeek 逐个调查时很容易连续产生 4 轮工具调用。现有实现实际只允许 3 轮工具调用，第 4 轮必须直接输出最终 JSON，因而与真实模型行为不匹配。
...
达到以下任一条件后进入 finalization：

- `toolRounds === maxToolRounds`
- `realToolCalls === maxRealToolCalls`
- 模型已经收集足够信息并返回无工具内容

达到工具轮次上限时，即使仍有剩余工具调用次数，也必须结束调查。应用先注入 `remaining_tool_rounds: 0` 的 `_budget_info`，再发起无工具请求。
...
DeepSeek adapter 在工具数组为空时必须从 HTTP body 中完全省略 `tools` 和 `tool_choice`。
...
- 同时存在正文和 `_budget_info`：移除 `_budget_info`，将正文作为最终输出验证。
- 只有 `_budget_info` 且无正文：视为空响应，在模型请求总上限内最多恢复一次。
...
type MessageToolName = AssistantToolName | ReservedControlToolName;
```
...
- 调查结束后的无工具 finalization。
- 安全事件和错误分类。
- 返回最终文本、sources 和调查统计。
...
- 只有 `request.tools.length > 0` 时发送 `tools` 和 `tool_choice: "auto"`。
- 最终汇总请求省略这两个字段。
...
说明固定 system prompt、虚拟预算消息、最终无工具汇总和安全 debug 开关。

## 测试计划

### `server/test/grounded-agent.test.ts`

- 连续 4 轮各调用 1 个真实工具，第 5 次生成最终 JSON。
...
- 最终请求的真实工具数组为空。
- 模型提前返回最终内容时不产生额外 finalization 请求。
...
- 正文和伪造 `_budget_info` 同时出现时仍验证正文。
- 仅返回伪造 `_budget_info` 时最多有界恢复一次。
...
- 非空工具请求继续包含 schema 和 `tool_choice: "auto"`。
- `finish_reason` 被正确解析。
...
- 4 个真实工具轮次后仍有一次独立最终汇总机会。
- 最终 DeepSeek 请求不携带 `tools` 或 `tool_choice`。
- 工具调用预算在执行前强制检查，不发生部分超支执行。
- 现有事实范围、citation 和 Widget 契约保持不变。
