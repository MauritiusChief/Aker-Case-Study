...
The assistant loop separates model attempts, business investigation rounds, business tool calls, and non-citable Widget draft calls. Every model response must call a tool. Business tools produce citable `tool_N` sources; Widget CRUD tools transactionally manage semantic Widget state; `submit_morning_brief` and `submit_assistant_answer` terminally submit only grounded text and citations. DeepSeek receives `tool_choice: "required"` during normal work and a named submission choice when work budgets are exhausted. After each work round the application injects a reserved `_budget_info` tool-result pair describing the remaining budget; this reserved tool is never registered, executed, or cited. Set `AKER_LLM_DEBUG=true` to log safe metadata without API keys, prompts, or business data.
